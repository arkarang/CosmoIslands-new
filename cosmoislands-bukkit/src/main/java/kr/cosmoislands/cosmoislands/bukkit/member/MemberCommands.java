    ...
    protected static class User extends BaseCommand {
            ...
            preconditions.hasIsland().thenCompose(hasIsland -> {
                ...
                if (isOwner != null) {
                    ...
                    if (isOwner) {
                        ...
                        toTransferPreconditions.getIsland().thenAccept(toTransferIsland -> {
                            preconditions.getIsland()
                                    .thenCompose(island -> {
                                        if (toTransferIsland == null || toTransferIsland.getId() != island.getId()) {
                                            player.sendMessage("해당 플레이어는 섬원이 아닙니다.");
                                            return CompletableFuture.completedFuture(null);
                                        } else {
                                            return provideGUI(island, player.getUniqueId(), toTransfer);
                                        }
                                    }).thenAccept(gui -> {
                                if (gui != null) {
        ...
        public void invite(Player player, String name) {
            ...
            preconditions.getIsland().thenCombine(receiverUuidFuture, (island, receiverUuid) -> {
                ...
                return CompletableFuture.completedFuture(false);
            }).thenCompose(future->future).thenCombine(receiverUuidFuture, (canExecute, receiverUuid)->{
                if(canExecute){
                    return PlayerPreconditions.of(receiverUuid);
                }else{
                    return null;
                }
            }).thenCombine(isOnlineFuture, (toReceivePrecondition, isOnline) -> {
                CompletableFuture<Island> result = CompletableFuture.completedFuture(null);

                if (toReceivePrecondition != null) {
                    result = toReceivePrecondition.hasIsland().thenCompose(hasIsland -> {
                        if (hasIsland) {
                ...
                return result;
            }).thenCompose(future -> future).thenCombine(receiverUuidFuture, (island, receiverUuid) -> {
                if (island == null) {
                    return null;
                }
        ...
        public void inviteIntern(Player player, String name) {
            ...
            }).thenCombine(isOnlineFuture, (toTransferPreconditions, isOnline) -> {
                CompletableFuture<Island> result = CompletableFuture.completedFuture(null);
        ...
        @Subcommand("알바 탈퇴")
        public void exitIntern(Player player, @Default("!here") String name){
            CompletableFuture<UUID> getUniqueIdFuture;
            boolean skipFoundingUniqueId = name.equals("!here");
            CompletableFuture<Island> targetIslandFuture = null;

            if(skipFoundingUniqueId){
                try {
                    Island island = IslandPreconditions.of(player.getWorld()).getIsland();
                    targetIslandFuture = CompletableFuture.completedFuture(island);
                }catch (IllegalArgumentException e){
                    player.sendMessage("당신이 서 있는 곳은 섬이 아닙니다.");
                }
            }else{
                getUniqueIdFuture = playersModule.getUniqueID(name);
                targetIslandFuture = getUniqueIdFuture.thenCompose(uuid->{
                    return PlayerPreconditions.of(uuid).getIsland();
                });
            }

            if(targetIslandFuture != null) {
                targetIslandFuture.thenCompose(island -> {
                    if (island == null) {
                        player.sendMessage("해당 플레이어는 섬에 소속되어 있지 않습니다.");
                        return CompletableFuture.completedFuture(null);
                    } else {
                        return IslandPreconditions.of(island).isInternOf(player.getUniqueId());
                    }
                }).thenCombine(targetIslandFuture, (isIntern, island)->{
                    if(isIntern != null){
                        if(isIntern){
                            IslandPlayersMap playersMap = island.getComponent(IslandPlayersMap.class);
                            ...
                            player.sendMessage("당신은 섬의 알바원이 아닙니다.");
                        }
                    }
                    return null;
                });
