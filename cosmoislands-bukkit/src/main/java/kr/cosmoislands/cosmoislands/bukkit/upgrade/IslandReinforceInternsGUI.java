...
public class IslandReinforceInternsGUI extends ReinforceGUI<IslandData>{

    IslandBank bank;

    public IslandReinforceInternsGUI(ReinforceSetting setting, IslandBank bank, IslandData islandData) {
        super("알바", setting, islandData);
        this.bank = bank;
        for(int i = 0; i < setting.getMaxLevel(); i++) {
            ...
            icons.put(i, () -> {
                ...
                return new ItemStackBuilder(item).setName("섬 최대 알바 : " + comp.getValue() + "명").addLine("§a§l 가격 §f: §e" + getCost(level) + "G").getHandle();
            });
        }
        init();
    }
    ...
    void executeNotEnoughLevel(Player player, int cl) {
        closeAndSend(player, "섬 레벨이 충족되지 않았습니다. 요구 알바 레벨: " + cl);
    }
